...
- 時刻は「**時（0〜23の24時間表記）**」＋「**分（00/15/30/45）**」を選びます。**時を選ばなければ 21:00**、分を選ばなければ 0 分になります。
- **お休みの日は「休業」をオン**にすると、その日が休業（終日）表示になります。
...
1. Discordで次のように入力して送信（月・日・時・分は一覧から選べます）:
   ```
   /予定  月:6  日:20  タイトル:雑談配信  時:21  分:00
   ```
   深夜帯もOK（例：`時:1 分:30` → 翌1:30）。時・分を省略すると 21:00 になります。
...
### 休業日（お休み）を入れたいとき
```
/予定  月:6  日:17  タイトル:お休み  休業:オン
```
「休業」をオンにすると、その日が「休業」表示になります（時・分は無視されます）。
