...
Googleカレンダー（HPが読んでいるのと同じカレンダー）
   ▼
HPの週間ボード（今週・来週）に自動反映（最大15分）
```

- **書き込みは「追加」だけ**です（編集・削除・双方向同期はしません）。
- 間違えたときは、Googleカレンダーを直接開いて消す/直すのが確実です。
- 時間を空のまま送ると「**休業**」として登録されます（その日のボードがグレーの休業表示になります）。
...
## パートD：コマンドを登録する（最初に一度だけ）

ローカル（自分のPC）で次を実行します。値は環境変数で渡すだけ＝**コードには残りません**。
...
1. Discordで次のように打って送信:
   ```
   /予定  日付:6/20  タイトル:雑談配信  時間:21:00  おすすめ:はい
   ```
2. 「✅ 配信予定を登録しました…」と自分だけに見える返信が来ればOK。
3. Googleカレンダーの6/20を見ると予定が入っています。
4. HPの週間ボードには次回更新（最大15分）で反映されます。すぐ見たい場合は再デプロイ。

### 休業日を入れたいとき
```
/予定  日付:6/17  タイトル:お休み
```
時間を空のまま送ると、その日が「休業」表示になります。
...
- **カレンダーに入ったのにHPに出ない** → 反映はビルド時（最大15分ごと）。急ぐなら再デプロイ。`ICS_URL` と `GOOGLE_CALENDAR_ID` が同じカレンダーか確認。
