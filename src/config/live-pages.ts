 ...
 *
 * ねらい：Xに貼ると「大きい画像カード」が出て、人間がクリックすると即YouTubeへ転送される中継ページ。
 * ・X/Discord等のカード取得ロボットはJSを実行しない → このページのOGタグ（大きいカード）を読む。
 * ・人間のブラウザはJSで即YouTubeへ転送される（[slug].astro のスクリプト）。
 * ・カード左下のドメインは wakamatsu-iori.com になる（Xの仕様上 youtube.com にはできない）。
 *
 * 使い方：
...
export interface LivePage {
  ...
  /** カードの説明文（1〜2文） */
  description: string;
  /** YouTubeの視聴URL（配信前は待機所URLでOK）。人間はここへ即転送される */
  youtubeUrl: string;
